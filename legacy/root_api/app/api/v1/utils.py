...
async def process_resource(resource_dict: Dict, session) -> Dict:
    ...
    try:
        ...
        relationships = await relationship_service.get_resource_relationships(
            resource_dict["id"], session
        )
        ...
        summaries_result = await session.execute(
            summaries_query, {"resource_id": resource_dict["id"]}
        )
        ...
        from app.api.v1.endpoint_modules.utils import (
            clean_dict,
            format_file_size,
            map_to_aardvark_fields,
        )

...
def populate_resource_distributions():
    ...
    This migration extracts distribution data from the dct_references_s JSON field
    ...
    try:
        ...
        with engine.connect() as conn:
            # First, let's check if we have any resources with dct_references_s
                ...
                try:
                    ...
                    if isinstance(dct_references_s, str):
                        try:
                            ...
                            logger.warning(f"Failed to parse JSON for resource {resource_id}: {dct_references_s[:100]}...")
                            error_count += 1
                            continue
                    else:
                    ...
                    if not isinstance(references, dict):
                        logger.warning(f"References not a dict for resource {resource_id}: {type(references)}")
                        error_count += 1
                        continue
                    ...
                    for uri, url_data in references.items():
                        if uri in uri_to_type_id:
                                ...
                                for item in url_data:
                                    ...
                                    if actual_url:  # Only insert if we have a valid URL
                                        conn.execute(text("""
                                            ...
                                            (resource_id, distribution_type_id, url, label, position)
                                            VALUES (:resource_id, :distribution_type_id, :url, :label, :position)
                            ...
                            logger.debug(f"Unknown distribution URI for resource {resource_id}: {uri}")

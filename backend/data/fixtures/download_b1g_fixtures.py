...
def main():
    ...
    # Read CSV and extract links with "b1g" in URL
    links = []
    seen_ids = set()
    with open(csv_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if link and 'b1g' in link.lower():
                # Extract ID from URL to avoid duplicates
                ...
                if path_parts:
                    ...
                    if record_id not in seen_ids:
                        ...
                        raw_link = f"{link}/raw"
                        links.append((raw_link, row.get('Title of Record', ''), record_id))
    
    print(f"\nFound {len(links)} unique b1g links to download\n")
    
    # Download each file
    successful = 0
    failed = 0
    
    for index, (link, title, record_id) in enumerate(links, 1):

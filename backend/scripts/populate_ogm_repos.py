...
def _github_headers(token: Optional[str]) -> Dict[str, str]:
    headers = {
        ...
        "User-Agent": "OpenGeoMetadata-API/1.0",
    }
    if token:
...
def build_repo_row(repo: Dict[str, Any], *, has_aardvark: bool) -> Dict[str, Any]:
    ...
    # - otherwise enable and default to weekly (can be edited via admin endpoint)
    ogm_enabled = bool(has_aardvark and not archived)
    ogm_watch_mode = "weekly" if ogm_enabled else "manual"

...
@pytest.mark.unit
def test_ogm_repo_filter_maps_to_include_filters():
    svc = SearchService()
    include_filters, exclude_filters = svc.extract_new_style_filters("ogm_repo=edu.unr")
    assert exclude_filters == {}
    assert include_filters.get("ogm_repo") == ["edu.unr"]

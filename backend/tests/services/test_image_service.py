...
class TestIssue412ThumbnailSources:
    ...
    def test_pennsylvania_image_keeps_provider_and_identifier(self, source):
        service = ImageService({})
        expected = "https://cdm17287.contentdm.oclc.org/iiif/2/wpamaps:2730/"
        expected += "info.json" if source.endswith("info.json") else "full/!800,800/0/default.jpg"
        assert service._get_thumbnail_source_url({"http://iiif.io/api/image": source}) == expected
    ...
    def test_contentdm_manifest_is_resolved_in_worker(self, path):
        ...
        service = ImageService({})
        with patch.object(service, "_queue_thumbnail_processing") as queue:
            assert (
                service._get_thumbnail_source_url(
                    {"http://iiif.io/api/presentation#manifest": manifest_url}
                )
